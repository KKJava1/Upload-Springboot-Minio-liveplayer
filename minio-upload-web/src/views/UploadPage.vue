...
import { taskInfo, initTask, preSignUrl, merge,selectTask } from '../lib/api';
...
const getTaskInfo = async (file) => {
    ...
    console.log("返回的结果为code,data,msg",code + data + msg)
...
const selectList = () =>{
  selectTask().then((res)=>{
    ...
    minioList.value = res.data
    videoUrl.value = res.data.map(item => item.url)
...
<template>
  ...
  <div>
    <el-card
        v-for="(url, index) in videoUrl"
        :key="index"
        style="width: 80%; margin: 80px auto"

    >
      <LivePlayer :videoUrl="url" :src="url" :autoplay="true"></LivePlayer>
    </el-card>
  </div>
</template>
